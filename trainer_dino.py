...
class BYOLTrainer:
    ...
    def entropy_dino_loss(self, s, t, tps=0.1, tpt=0.04):
        t = torch.softmax((t - self.C) / tpt, dim=1)  # center + sharpen

        m = 0.92
        self.C = m * self.C + (1 - m) * t.mean(dim=0)
        return - (t * torch.log_softmax(s / tps, dim=-1)).sum(dim=1).mean()
    ...
    def update(self, batch_view_1, batch_view_2):
        ...
        loss = self.entropy_dino_loss(predictions_from_view_1, targets_to_view_1)
        loss += self.entropy_dino_loss(predictions_from_view_2, targets_to_view_2)
